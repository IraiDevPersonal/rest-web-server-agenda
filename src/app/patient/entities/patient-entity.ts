...
export class PatientEntity {
  static validate(item: any): PatientModel {
    try {
      const data = PatientEntity.mapper(item);
      return PatientSchema.parse(data);
    } catch (error) {
      throw new Error(
        CustomError.getErrorMessage(error, "patient-entity.ts: (validate)")
      );
    }
  }
  ...
  static serverResponse(item: any): PatientModel[] {
    try {
      ...
      }).map(PatientEntity.validate);
    } catch (error) {
      throw new Error(
        CustomError.getErrorMessage(
          error,
          "patient-entity.ts: (serverResponse)"
        )
      );
    }
  }
