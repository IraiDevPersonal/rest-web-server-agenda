...
export class ServiceProviderCodesMapper {
  private static _mapper(item: BdServiceProviderCodes): ServiceProviderCodesModel {
    return {
      id: item.id,
      code: item.code,
      title: item.title,
  ...
  static validate(item: BdServiceProviderCodes): ServiceProviderCodesModel {
    try {
      const data = ServiceProviderCodesMapper._mapper(item);
      return ServiceProviderCodesScheme.parse(data);
    } catch (error) {
      throw CustomError.internalServer(
        CustomError.getErrorMessage(error, "service-provider-codes-mapper.ts (validate)")
      );
    }
  }
  ...
  static toArray(data: BdServiceProviderCodes[]): ServiceProviderCodesModel[] {
    return safeArray(data, {
      errorMessage: "service-provider-codes-mapper.ts (toArray): se esperaba un array"
